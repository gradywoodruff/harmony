...
function NC_GetScriptFile(fileName) {
  ...
  for (i in configFileLines) {
    ...
    if (selLine_clean.indexOf("USA_DB") > -1) {
      globalScriptsFolder =
        "/" + selLine_clean.split(" /")[1] + "/scripts" //the server file path will not be split even if the server is named with any ' ' characters
...
function NC_GetAllDstNodes(
  ...
  for (
    var i_port = 0;
    i_port < node.numberOfOutputPorts(selNode);
    i_port++
  ) {
    // for each output port get all of the output links
    for (
      var i_link = 0;
      i_link < node.numberOfOutputLinks(selNode, i_port);
      i_link++
    ) {
      ...
      var nodeTypeWeWant =
        nodeTypes_toManipulate.indexOf(selDstNode_type) > -1
      ...
      if (nodeIsNotListed && nodeTypeWeWant) {
        ...
        var nodesBelowSelDstNode = NC_GetAllDstNodes(
          nodeTypes_toManipulate,
          dstNodeCollection,
          selDstNode
        )
        ...
        for (i in nodesBelowSelDstNode) {
          ...
          var nodeBelowIsNotListed =
            dstNodeCollection.indexOf(selBelowNode) == -1
